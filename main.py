...
def showAllClasses():
    cur.execute("SELECT class_name, class_date, class_time, trainer_id, room_id, class_id FROM classes")
    ...
    for row in classes:
        ...
        room = cur.fetchone()
        print(str(row[5]) + ") " + row[0] + " run by " + trainer[0] + " " + trainer[1] + " in " + room[0] + " at " + str(row[2]) + " on " + str(row[1]))
...
def addNewEvent():
    ...
    cur.execute("INSERT INTO events (event_name, event_date, event_time, room_id) VALUES (%s, %s, %s, %s);", [eventName, eventDate, eventTime, eventRoom])
    conn.commit()
